...
const review_entity_1 = require("../src/modules/reviews/entities/review.entity");
const discount_entity_1 = require("../src/modules/discounts/entities/discount.entity");
async function run() {
    const app = await core_1.NestFactory.createApplicationContext(app_module_1.AppModule);
    try {
        const users = app.get(users_service_1.UsersService);
        const productModel = app.get((0, mongoose_1.getModelToken)(product_entity_1.Product.name));
        const categoryModel = app.get((0, mongoose_1.getModelToken)(category_entity_1.Category.name));
        const reviewModel = app.get((0, mongoose_1.getModelToken)(review_entity_1.Review.name));
        const discountModel = app.get((0, mongoose_1.getModelToken)(discount_entity_1.Discount.name));
        ...
        const ensureUser = async (email, displayName) => {
            ...
            if (existing)
                return existing;
            return users.createUser({ email, password: 'user123', displayName }, role_enum_1.Role.USER);
        };
        ...
        for (const c of categorySeeds) {
            ...
            if (found) {
                categoryMap.set(c.name, found._id);
            }
            else {
                const created = await categoryModel.create({ name: c.name, icon: c.icon });
                categoryMap.set(c.name, created._id);
            }
        }
        const products = [
            {
        ...
        for (const p of products) {
            ...
            if (exists) {
                await productModel.updateOne({ _id: exists._id }, { $set: { categoryId: categoryId ?? undefined, createdBy: exists['createdBy'] ?? adminId } }).exec();
                productDocs.push({ _id: exists._id, name: exists.name, category: exists['category'] ?? '' });
            ...
            productDocs.push({ _id: created._id, name: created.name, category: created.category ?? '' });
            createdCount++;
